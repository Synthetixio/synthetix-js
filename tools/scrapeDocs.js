...
const contractNames  = ['Havven', 'Nomin', 'IssuanceController'];
const docsObj = {};
...
async function getDocsForContract(contractName) {
  ...
  $('.item').each((index, item) => {
    ...
    const description = $(item).find('.body > .description > p').text();
...
async function start(){
  try{
    ...
    console.log(docsObj)
    await fs.writeFile(__dirname + '/../lib/docSrc/descriptions.json', JSON.stringify(docsObj, null, 2));
    console.log('done');
  } catch (e){
    console.log(e)
  }
