...
class LedgerSigner {
  ...
  async sign(transaction) {
    ...
    const signature = await this.eth.signTransaction(LEDGER_DERIVATION_PATH, rawTx);
    const txToSerialize = {
      ...txFields,
  ...
  async getNextAddresses(fromAddressIndex, pageSize) {
    try {
      ...
      for (let i = 0; i < pageSize; i++) {
        let res = await this.eth.getAddress(LEDGER_DERIVATION_PATH + (fromAddressIndex + i));
